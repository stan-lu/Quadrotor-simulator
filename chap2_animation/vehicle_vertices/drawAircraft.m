...
py = zeros(num_quad,1);
pz = zeros(num_quad,1);
phi = zeros(num_quad,1);
theta = zeros(num_quad,1);
psi = zeros(num_quad,1);
...
persistent field_view_handle_2
...
if t==0
    ...
    for i = 1:num_quad
        vehicle_handle(i) = drawVehicleBody(Vertices,Faces,facecolors,...
            px(i),py(i),pz(i),phi(i),theta(i),psi(i),...
            [],'normal');
    end
    ...
    view(68,64)  % set the vieew angle for figure
...
%     axis([-2,4,-1,5,-2,5]);
    ...
    for i = 1:num_quad
        Vert_field_view = findFieldOfView(px(i),py(i),pz(i),phi(i),theta(i),psi(i),P);
        [field_view_handle(i),field_view_handle_2(i)]...
            = drawFieldOfView(px(i),py(i),pz(i),Vert_field_view,[],[]);
    end
    hold on
    
    h_pos_handle = drawTrajectory(px(1), py(1), pz(1), 'b:', []);
    %     h_pos_handle(2) = drawTrajectory(px(2), py(2), pz(2), 'b:', []);
    hold on
    ...
    for i = 1:num_quad
        ...
        Vert_field_view = findFieldOfView(px(i),py(i),pz(i),phi(i),theta(i),psi(i),P);
        drawFieldOfView(px(i),py(i),pz(i),Vert_field_view,field_view_handle(i),field_view_handle_2(i));
        
    end
    
    drawTrajectory(px(1), py(1), pz(1), 'b:', h_pos_handle);
    %     drawTrajectory(px(2), py(2), pz(2), 'b:', h_pos_handle(2));
