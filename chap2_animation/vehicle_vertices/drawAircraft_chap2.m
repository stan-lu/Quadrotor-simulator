...
function drawAircraft_chap2(uu,P)

% process inputs to function

px       = uu(1);       % inertial x position
py       = uu(2);       % inertial y position
pz       = uu(3);
phi      = uu(7);       % roll angle
...
%     axis([-11,0.3,-5.3,5.3,-2,8]);
%     axis([-1,1,-1,1,0,3.5]);
    axis off
    hold on
    
%     Vert_field_view = findFieldOfView(px,py,pz,phi,theta,psi,P);
    [field_view_handle,field_view_handle_2]...
    ...
    drawVehicleBody(Vertices,Faces,facecolors,...
        ...
        vehicle_handle);
%     Vert_field_view = findFieldOfView(px,py,pz,phi,theta,psi,P);
    drawFieldOfView(px,py,pz,Vert_field_view,field_view_handle,field_view_handle_2);
...
if isempty(handle)
    handle   = plot3(XYZ(1,:),XYZ(2,:),XYZ(3,:),'k','LineWidth',1.5);
    handle_2 = plot3(XYZ_2(1,:),XYZ_2(2,:),XYZ_2(3,:),':','Color',[0.4 0.4 0.4], 'LineWidth',2);
else
